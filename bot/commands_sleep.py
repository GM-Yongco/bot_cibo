...
from functions_sleep import sleep_post, sleep_get_logs, sleep_get_status
...
def define_commands_sleep(bot:discord.ext.commands.bot.Bot) -> None:
	...
	async def log_sleep(
		...
		await interaction.response.send_message(sleep_post(
			hour_sleep_start=hour_sleep_start, 
			hour_sleep_end=hour_sleep_end
		))
	...
	async def get_stats(interaction: discord.Interaction):
		...
		await interaction.response.send_message(f"```{sleep_get_logs()}```")
	...
	async def get_status(interaction: discord.Interaction):
		...
		await interaction.response.send_message(f"```{sleep_get_status()}```")